...
export interface RefundRequest {
  ...
  reason: 'requested_by_customer' | 'duplicate_payment' | 'fraudulent' | 'service_not_provided' | 'technical_issue' | 'billing_error' | 'other';
...
export class RefundManager {
  ...
  async processRefund(request: RefundRequest): Promise<RefundResult> {
    ...
    try {
      ...
      const eligibility = await this.checkRefundEligibility(request.organizationId, request.subscriptionId);
      ...
      await this.sendRefundNotifications(request.organizationId, refundId, refundAmount, request.currency);
  ...
  private async calculateRefundAmount(
    ...
    if (request.refundType === 'prorated') {
      ...
      const proratedAmount = Math.floor(
        (request.amount * remainingPeriod) / totalPeriod
      );
