...
export class PaymentIntentManager {
   ...
   */
  async createPaymentIntentWithSCA(request: PaymentIntentRequest): Promise<PaymentIntentResult> {
    ...
    try {
      ...
      await this.logComplianceValidation(dbPaymentIntentId, scaExemption, request)
   ...
   */
  async confirmPaymentIntent(request: PaymentConfirmationRequest): Promise<PaymentIntentResult> {
    const supabase = await createClient()
    ...
    try {
      ...
      // Log failure
      await this.handleAuthenticationFailure(request.paymentIntentId, error, 1)
   ...
   */
  async handle3DSAuthentication(clientSecret: string): Promise<AuthenticationResult> {
    try {
  ...
  private async savePaymentIntentRecord(data: {
    ...
    const { data: result, error } = await supabase.rpc('create_payment_intent_record', {
   ...
   */
  async getAuthenticationStatistics(startDate?: string, endDate?: string): Promise<any> {
    const supabase = await createClient()
