...
import { SAMLConfig, SAMLAssertion, SSOConfiguration, SSOError, SPMetadata } from './types'
...
export class SAMLHandler {
  ...
  constructor(config: SSOConfiguration) {
    ...
    this.samlConfig = {
      ...
      nameIdFormat:
        config.saml_name_id_format || 'urn:oasis:names:tc:SAML:1.1:nameid-format:emailAddress',
    }
  }
...
<samlp:AuthnRequest xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol"
  ...
  <samlp:NameIDPolicy Format="${this.samlConfig.nameIdFormat}" AllowCreate="true"/>
    ...
    // Build redirect URL
    const url = this.buildRedirectUrl(this.samlConfig.ssoUrl, encodedRequest, relayState)
   ...
   */
  async processResponse(samlResponse: string, relayState?: string): Promise<SAMLAssertion> {
    try {
      // Decode response
      ...
      throw this.createSSOError('INVALID_ASSERTION', 'Failed to process SAML response', {
  ...
  async generateLogoutRequest(
    ...
    if (!this.samlConfig.sloUrl) {
      throw this.createSSOError('CONFIG_NOT_FOUND', 'Single Logout URL not configured')
    }
...
<md:EntityDescriptor xmlns:md="urn:oasis:names:tc:SAML:2.0:metadata"
  ...
  private buildRedirectUrl(destination: string, request: string, relayState?: string): string {
  ...
  private async extractAssertion(parsedResponse: any): Promise<SAMLAssertion> {
    try {
      ...
      return {
        ...
        notOnOrAfter: conditions?.$.NotOnOrAfter ? new Date(conditions.$.NotOnOrAfter) : undefined,
  ...
  private async validateAssertion(assertion: SAMLAssertion): Promise<void> {
    ...
    if (assertion.notBefore && now < assertion.notBefore) {
      throw this.createSSOError('INVALID_ASSERTION', 'Assertion not yet valid', {
        notBefore: assertion.notBefore.toISOString(),
      })
    }

    if (assertion.notOnOrAfter && now >= assertion.notOnOrAfter) {
      throw this.createSSOError('INVALID_ASSERTION', 'Assertion has expired', {
        notOnOrAfter: assertion.notOnOrAfter.toISOString(),
      })
    }

    // Validate audience
    if (assertion.audience && assertion.audience !== this.samlConfig.entityId) {
 ...
 */
export async function createSAMLHandler(configId: string): Promise<SAMLHandler> {
  const supabase = await createClient()
