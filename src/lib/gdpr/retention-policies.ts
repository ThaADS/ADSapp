...
export async function getRetentionPolicy(
  ...
  if (
    ...
    console.warn(`[RetentionPolicy] Override for ${dataType} below minimum, using default`)
    return defaultPolicy
  }
  ...
  if (
    ...
    console.warn(`[RetentionPolicy] Override for ${dataType} above maximum, using default`)
    return defaultPolicy
  }
...
export async function createRetentionOverride(
  ...
  if (!validation.valid) {
    throw new Error(`Invalid retention override: ${validation.errors.join(', ')}`)
  }
...
export async function validateRetentionOverride(
  ...
  if (defaultPolicy.minimumRetentionDays && retentionDays < defaultPolicy.minimumRetentionDays) {
    errors.push(
  ...
  const percentDifference =
    Math.abs(retentionDays - defaultPolicy.retentionDays) / defaultPolicy.retentionDays
...
export async function getExpiredRecords(
        ...
        for (const msg of messages) {
          ...
          expiredRecords.push({
            ...
            daysOverdue: Math.floor(
              (Date.now() - new Date(msg.created_at).getTime()) / (1000 * 60 * 60 * 24) -
    ...
    case DataType.CONTACTS: {
      ...
      if (contacts) {
        for (const contact of contacts) {
          const lastActivity = new Date(contact.last_message_at || contact.updated_at)
          const expirationDate = await calculateExpirationDate(
            dataType,
            lastActivity,
            organizationId
          ...
          expiredRecords.push({
            ...
            expirationDate: expirationDate?.toISOString() || '',
            daysOverdue: Math.floor(
              (Date.now() - lastActivity.getTime()) / (1000 * 60 * 60 * 24) - policy.retentionDays
            ),
          })
        }
      }
  ...
  console.log(`[RetentionPolicy] Found ${expiredRecords.length} expired ${dataType} records`)
 ...
 * @returns Validation result
 */
export async function validateAllPolicies(organizationId: string): Promise<PolicyValidationResult> {
  const errors: string[] = []
  const warnings: string[] = []
  const conflicts: PolicyConflict[] = []
  ...
  const messagesPolicy = await getRetentionPolicy(DataType.MESSAGES, organizationId)
  const conversationsPolicy = await getRetentionPolicy(DataType.CONVERSATIONS, organizationId)
