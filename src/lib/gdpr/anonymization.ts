...
export function getAnonymizedValue(
  ...
  switch (type) {
    ...
    case AnonymizationType.USER_AGENT: {
      ...
      const browserMatch = value.match(/(Chrome|Firefox|Safari|Edge|Opera)/i)
 ...
 */
export function anonymizePhoneNumber(phoneNumber: string, organizationId: string): string {
 ...
 */
export function anonymizeEmail(email: string, organizationId: string): string {
  const salt = getOrganizationSalt(organizationId)
...
export function generateAnonymizationReport(entries: AnonymizationEntry[]): {
  ...
  entries.forEach(entry => {
    ...
    // Quality check
    const quality = verifyAnonymizationQuality(entry.original, entry.anonymized)

    if (quality.secure) {
      qualityChecks.passed += 1
    } else {
      qualityChecks.failed += 1
      qualityChecks.warnings.push(`Quality issue with ${entry.type}: ${quality.details.join(', ')}`)
    }
  })
