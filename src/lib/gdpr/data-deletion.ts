...
export class DataDeletionService {
  ...
  static async createDeletionRequest(
    ...
    console.log(`[DataDeletion] Created deletion request ${data.id} for ${input.request_type}`)
  ...
  static async deleteUserAccount(
    ...
    console.log(`[DataDeletion] Deleting user account ${userId} (soft: ${softDelete})`)
  ...
  static async deleteContactData(
    ...
    try {
      ...
      console.log(`[DataDeletion] Deleted contact ${contactId}: ${summary.total} records`)
  ...
  static async deleteConversationData(
    ...
    console.log(`[DataDeletion] Deleting conversation data for contact ${contactId}`)
  ...
  static async softDelete(
    ...
    // Validate table name
    const validTables = ['profiles', 'contacts', 'conversations', 'messages', 'organizations']
  ...
  static async hardDelete(
    ...
    if (!validTables.includes(tableName)) {
      throw new Error(`Hard delete not allowed for table: ${tableName}. Use soft delete.`)
    }
   ...
   */
  static async getDeletionRequest(requestId: string): Promise<DeletionRequest | null> {
    const supabase = await createClient()
