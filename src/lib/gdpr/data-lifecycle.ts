...
export async function scheduleDataDeletion(
  ...
  console.log(`[GDPR] Deletion scheduled for user ${userId}, execution in ${gracePeriodDays} days`)
...
export async function executeDataDeletion(deletionRequestId: string): Promise<DeletionResult> {
  ...
  if (deletionRequest.status !== DeletionStatus.SCHEDULED) {
    throw new Error(`Cannot execute deletion with status: ${deletionRequest.status}`)
  }
  ...
  try {
    ...
    const { count: messagesDeleted, error: messagesError } = await supabase
      ...
      .in(
        'conversation_id',
        supabase.from('conversations').select('id').eq('organization_id', organizationId)
      )
    ...
    const { count: conversationsDeleted, error: conversationsError } = await supabase
      .from('conversations')
      .delete()
      .eq('organization_id', organizationId)

    if (conversationsError) {
      errors.push(`Conversations deletion failed: ${conversationsError.message}`)
    } else {
    ...
    await supabase
      .from('deletion_requests')
      .update({
        status: verificationResult.verified ? DeletionStatus.COMPLETED : DeletionStatus.FAILED,
    ...
    console.log(`[GDPR] Deletion completed for user ${userId}: ${recordsDeleted} records deleted`)
...
export async function verifyDeletion(
  ...
  const tablesToCheck = ['profiles', 'contacts', 'conversations', 'messages', 'billing_events']
