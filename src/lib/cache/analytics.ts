...
export class CacheAnalytics {
  ...
  private calculateTrends(): {
    ...
    const olderHitRate = older.reduce((sum, m) => sum + m.combined.overallHitRate, 0) / older.length

    // Calculate average latencies
    const recentLatency =
      recent.reduce((sum, m) => sum + m.combined.averageLatency, 0) / recent.length
    const olderLatency = older.reduce((sum, m) => sum + m.combined.averageLatency, 0) / older.length
    ...
    return {
      hitRateTrend: hitRateDiff > 5 ? 'improving' : hitRateDiff < -5 ? 'declining' : 'stable',
      latencyTrend: latencyDiff < -10 ? 'improving' : latencyDiff > 10 ? 'declining' : 'stable',
    }
  }
