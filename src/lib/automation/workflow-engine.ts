...
export class WorkflowExecutionEngine {
  ...
  private async executeNode(
    ...
    context: ExecutionContext
  ): Promise<{ success: boolean; output?: Record<string, any>; branch?: 'true' | 'false' | 'success' | 'failure' }> {
    switch (node.type) {
      case 'trigger':
  ...
  private async executeConditionNode(
    ...
    switch (condition_type) {
      case 'keyword_match':
        ...
        const keywords = Array.isArray(node.config.keywords) ? node.config.keywords : [node.config.keyword];
  ...
  private async logNodeExecution(
    ...
    const { error } = await this.supabase
      ...
      .insert({
        ...
        ...logData,
        completed_at: logData.status === 'completed' || logData.status === 'failed'
