...
export class BillingReconciliationEngine {
  ...
  async generateInvoiceAdjustments(invoiceId: string): Promise<{
    ...
    try {
      ...
      if (Math.abs(discrepancy) > 100) {
        // Only adjust if difference is > $1.00
  ...
  async getBillingAnalytics(
    ...
    try {
      const [revenueMetrics, reconciliationStatus, churnAnalysis, complianceStatus] =
        await Promise.all([
  ...
  private async calculateChurnProbability(organization: any): Promise<ChurnPrediction> {
    ...
    const latePayments =
      organization.billing_transactions?.filter((t: any) => t.status === 'failed').length || 0
  ...
  private async getRecentUsageTrend(
    organizationId: string
  ...
  private async getChurnAnalysis(): Promise<any> {
    ...
    const avgProbability =
      data?.reduce((sum, p) => sum + p.churn_probability, 0) / (data?.length || 1)
