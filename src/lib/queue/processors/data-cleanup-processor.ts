...
import type { DataCleanupJobData, DataCleanupResult, DataType } from '@/lib/gdpr/types'
 ...
 */
export async function processDataCleanup(job: Job<DataCleanupJobData>): Promise<DataCleanupResult> {
    ...
    for (const org of organizations) {
      for (const dataType of dataTypes) {
        try {
          ...
          if (
            policy &&
            (!policy.is_active || !policy.enforcement_enabled || !policy.auto_delete_enabled)
          ) {
            continue
          }
          ...
          console.error(`[DataCleanup] Error scheduling cleanup for ${org.id}:${dataType}:`, err)
        }
      }
    }
 ...
 */
export async function generateCleanupReport(organizationId: string): Promise<{
  ...
  for (const dataType of dataTypes) {
    try {
      const stats = await RetentionPolicyEngine.getPolicyStats(organizationId, dataType)
