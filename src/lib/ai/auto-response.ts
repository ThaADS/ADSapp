...
export async function shouldAutoRespond(
  ...
  try {
    ...
    if (conditions.maxQueueTime) {
      ...
      if (conversation) {
        const waitTime = Date.now() - new Date(conversation.last_message_at || conversation.created_at).getTime();
        const maxWaitMs = conditions.maxQueueTime * 60 * 1000;

        if (waitTime > maxWaitMs) {
          return { should: true, reason: `Queue time exceeded (${Math.floor(waitTime / 60000)} min)`, config };
        }
      }
    }
...
Geef alleen de bericht tekst, geen uitleg of quotes.`;
  ...
  try {
    ...
    const fallbackMessage = config.language === 'nl'
