...
export class OpenRouterClient {
  ...
  async chat(
    ...
    try {
      ...
      console.log(
        `✅ OpenRouter success: ${model} (${latency}ms, ${response.usage.total_tokens} tokens)`
      )
      ...
      console.warn(
        `⚠️  Primary model ${model} failed after ${primaryLatency}ms, falling back to ${this.fallbackModel}`
      )
  ...
  private calculateCost(model: string, totalTokens: number): number {
    ...
    const costs: Record<string, number> = {
      ...
      'mistralai/mistral-small': 1.0,
    }

    const costPerMillion = costs[model] || 1.0 // Default fallback
