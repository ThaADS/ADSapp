...
export function ProfileSettings({ profile }: ProfileSettingsProps) {
  ...
  const formatRelativeTime = (dateString: string) => {
    ...
    if (diffMins < 60) return `${diffMins} ${diffMins > 1 ? t('profile.time.minutes') : t('profile.time.minute')} ${t('profile.time.ago')}`
    if (diffHours < 24) return `${diffHours} ${diffHours > 1 ? t('profile.time.hours') : t('profile.time.hour')} ${t('profile.time.ago')}`
    return `${diffDays} ${diffDays > 1 ? t('profile.time.days') : t('profile.time.day')} ${t('profile.time.ago')}`
  }
  ...
  return (
    <div className='space-y-6'>
          ...
          <div className='flex items-center space-x-5'>
            ...
            <div className='min-w-0 flex-1'>
              <h2 className='text-2xl font-bold text-gray-900 sm:truncate'>
                {profile.full_name || t('team.noNameSet')}
              </h2>
      ...
      <div className='rounded-lg border border-gray-200 bg-white shadow-sm'>
        <div className='p-6'>
          <h3 className='mb-4 text-lg font-semibold text-gray-900'>{t('profile.personalInfo')}</h3>
      ...
      <div className='rounded-lg border border-gray-200 bg-white shadow-sm'>
        <div className='p-6'>
          <h3 className='mb-4 text-lg font-semibold text-gray-900'>{t('profile.security')}</h3>

          <div className='space-y-4'>
            <div className='flex items-center justify-between'>
              <div>
                <h4 className='text-sm font-medium text-gray-900'>{t('profile.password')}</h4>
                <p className='text-sm text-gray-500'>{t('profile.lastUpdated')}: {t('profile.unknown')}</p>
              </div>
              <a
      ...
      <div className='rounded-lg border border-gray-200 bg-white shadow-sm'>
        <div className='p-6'>
          <div className='flex items-center gap-3 mb-4'>
            ...
            <h3 className='text-lg font-semibold text-gray-900'>{t('profile.mfa')}</h3>
          </div>
          ...
          {mfaLoading ? (
            <div className='flex items-center justify-center py-8'>
              ...
              <span className='ml-2 text-gray-500'>{t('profile.mfaLoading')}</span>
            </div>
          ) : mfaStatus?.enabled ? (
            <div className='space-y-4'>
              <div className='flex items-center gap-2'>
                <div className='h-3 w-3 rounded-full bg-emerald-500'></div>
                <span className='text-sm font-medium text-emerald-700'>{t('profile.mfaEnabled')}</span>
              </div>

              <div className='rounded-lg bg-gray-50 p-4'>
                <div className='grid grid-cols-1 gap-4 sm:grid-cols-2'>
                  <div>
                    <p className='text-xs text-gray-500'>{t('profile.enrolledAt')}</p>
                  ...
                  <div>
                    <p className='text-xs text-gray-500'>{t('profile.backupCodesRemaining')}</p>
                    <p className={`text-sm font-medium ${(mfaStatus.backupCodesRemaining || 0) <= 2
                      ? 'text-red-600'
                      : 'text-gray-900'
                      }`}>
                      {mfaStatus.backupCodesRemaining ?? t('profile.unknown')}
                      {(mfaStatus.backupCodesRemaining || 0) <= 2 && (
                        <span className='ml-2 text-xs text-red-600'>({t('profile.backupCodesLow')})</span>
                      )}
                    </p>
                  </div>
                </div>
            ...
            <div className='space-y-4'>
              <div className='flex items-center gap-2'>
                ...
                <span className='text-sm font-medium text-gray-500'>{t('profile.mfaDisabled')}</span>
              </div>
      ...
      <div className='rounded-lg border border-gray-200 bg-white shadow-sm'>
        <div className='p-6'>
          <div className='flex items-center justify-between mb-4'>
            <h3 className='text-lg font-semibold text-gray-900'>{t('profile.activeSessions')}</h3>
          ...
          {sessionsLoading ? (
            <div className='flex items-center justify-center py-8'>
              <div className='h-6 w-6 animate-spin rounded-full border-2 border-emerald-600 border-t-transparent'></div>
              <span className='ml-2 text-gray-500'>{t('profile.sessionsLoading')}</span>
            </div>
          ) : sessions.length === 0 ? (
            <p className='text-sm text-gray-500 py-4'>{t('profile.noSessions')}</p>
          ) : (
              ...
              {sessions.map(session => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between rounded-lg border p-4 ${session.isCurrent
                      ? 'border-emerald-200 bg-emerald-50'
                      : 'border-gray-200 bg-gray-50'
                      }`}
                  >
                    <div className='flex items-center gap-4'>
                      <DeviceIcon className={`h-8 w-8 ${session.isCurrent ? 'text-emerald-600' : 'text-gray-400'
                        }`} />
                      <div>
                        <div className='flex items-center gap-2'>
                          <p className='text-sm font-medium text-gray-900'>
                            {session.deviceInfo.platform || t('profile.unknownDevice')}
                          </p>
                          {session.isCurrent && (
                        ...
                        <p className='text-xs text-gray-500 mt-1'>
                          IP: {session.deviceInfo.ip} • {t('profile.lastActive')}: {formatRelativeTime(session.lastActivity)}
                        </p>
                      </div>
                    </div>
                    {!session.isCurrent && (
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50'>
          <div className='w-full max-w-md rounded-lg bg-white p-6 shadow-xl'>
            <div className='flex items-center gap-3 mb-4'>
              ...
              <h3 className='text-lg font-semibold text-gray-900'>{t('profile.confirmDisableMfa')}</h3>
            </div>
            ...
            <div className='flex justify-end gap-3'>
              ...
              >
                {mfaLoading ? t('profile.disabling') : t('profile.disableMfa')}
              </button>
            </div>
          </div>
        </div>
