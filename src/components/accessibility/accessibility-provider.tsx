...
function accessibilityReducer(
  ...
  switch (action.type) {
    ...
    case 'SET_LIVE_REGION':
      return {
        ...state,
        [action.payload.type === 'polite' ? 'liveRegionPolite' : 'liveRegionAssertive']:
...
export function AccessibilityProvider({ children }: { children: React.ReactNode }) {
  ...
  const actions = {
    ...
    setTheme: useCallback(
      (theme: AccessibilityState['theme']) => dispatch({ type: 'SET_THEME', payload: theme }),
      []
    ),
    setFontSize: useCallback(
      (size: AccessibilityState['fontSize']) => dispatch({ type: 'SET_FONT_SIZE', payload: size }),
      []
    ),
    announce: useCallback(
      (message: string) => dispatch({ type: 'ANNOUNCE', payload: message }),
      []
    ),
    setLiveRegion: useCallback(
      (type: 'polite' | 'assertive', message: string) =>
    ...
    setFocus: useCallback(
      (elementId: string | null) => dispatch({ type: 'SET_FOCUS', payload: elementId }),
      []
    ),
  ...
  const getFocusableElementsHelper = useCallback(
    (container: Element = document.body): Element[] => {
  ...
  const utils = {
    getAriaLabel: useCallback(
      (base: string, context?: string): string => {
    ...
    manageFocus: useCallback(
      (direction: 'next' | 'previous' | 'first' | 'last', container: Element = document.body) => {
        const focusableElements = getFocusableElementsHelper(container)
  ...
  return (
    <AccessibilityContext.Provider value={value}>
      ...
      <div aria-live='polite' aria-atomic='false' className='sr-only' role='log'>
