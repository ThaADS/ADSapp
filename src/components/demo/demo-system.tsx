    ...
    <>
      ...
      {state.isActive && (
        <>
          ...
          {state.currentStep === 0 && <NotificationWatermark autoHide={true} duration={10000} />}
        </>
      )}
    </>
  )
 ...
 */
export function withDemoSystem<P extends object>(WrappedComponent: React.ComponentType<P>) {
...
export function DemoMetrics() {
  ...
  return (
    <div className='rounded-lg border border-gray-200 bg-white p-4'>
      <h3 className='mb-3 text-sm font-semibold text-gray-900'>Demo Progress</h3>
      <div className='grid grid-cols-2 gap-4 text-center'>
        ...
        <div>
          <div className='text-2xl font-bold text-orange-600'>
            {state.analytics.interactionCount}
          </div>
...
export function DemoBreadcrumb() {
  ...
  return (
    <nav className='border-b border-gray-200 bg-white px-4 py-2' aria-label='Demo progress'>
...
export function DemoAnnouncer() {
  ...
  React.useEffect(() => {
    if (state.isActive) {
      ...
      if (currentStep) {
        setAnnouncement(
          `Demo step ${state.currentStep + 1} of ${state.steps.length}: ${currentStep.title}`
        )
      }
    }
