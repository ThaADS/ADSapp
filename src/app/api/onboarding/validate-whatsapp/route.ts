  ...
  try {
    ...
    if (businessAccountId) {
      ...
      return NextResponse.json({
        ...
        error: businessAccountIdValid ? undefined : 'Business Account ID must be 15-20 digits',
      })
    }
    ...
    if (accessToken) {
      ...
      const accessTokenValid = accessToken.startsWith('EAA') && accessToken.length > 100
    ...
    return NextResponse.json({ error: 'No validation field provided' }, { status: 400 })
  } catch (error) {
