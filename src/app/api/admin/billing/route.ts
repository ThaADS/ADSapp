...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (view === 'overview') {
      ...
      // Subscription distribution
      const subDistribution = (subscriptionDistribution.data || []).reduce(
        (acc, org) => {
      ...
      const activeSubscriptions = (subscriptionDistribution.data || []).filter(
      ...
      const trialsStarted30Days = (trialOrganizations.data || []).filter(
        org => new Date(org.created_at) >= thirtyDaysAgo
      ).length

      const trialConversionRate =
        trialsStarted30Days > 0 ? (trialConversions / trialsStarted30Days) * 100 : 0
      ...
      const overview = {
        ...
        trials: {
          ...
          expiringSoon: (trialOrganizations.data || []).filter(org => {
