...
interface RotationResponse {
  ...
  reEncryptionResult?: Record<string, {
    table: string;
    total: number;
    processed: number;
    successful: number;
  ...
  try {
    ...
    if (tenantId) {
      ...
      if (dryRun) {
        ...
        warningDate.setDate(
          warningDate.getDate() +
            parseInt(process.env.KEY_ROTATION_WARNING_DAYS || '7')
        );
        ...
        const tenantIds = [
          ...new Set(keysNeedingRotation?.map((k) => k.tenant_id) || []),
        ];
    ...
    return NextResponse.json(
      {
        ...
        nextActions: health.stats.keysNearExpiration > 0
