  ...
  try {
    ...
    try {
      ...
      if (!kmsConnected) {
        ...
        response.recommendations.push(
          'Check KMS credentials and network connectivity'
        );
      }
    ...
    try {
      ...
      if (stats.expiredKeys > 0) {
        ...
        response.recommendations.push(
          'Trigger immediate key rotation for expired keys'
        );
      }
      ...
      if (stats.pendingRotation > 5) {
        ...
        response.checks.keys.issues.push(
          `${stats.pendingRotation} keys approaching expiration`
        );
        if (response.status === 'healthy') {
          response.status = 'degraded';
        }
        response.recommendations.push(
          'Schedule key rotation for keys approaching expiration'
        );
      }
      ...
      if (stats.totalKeys === 0) {
        ...
        response.recommendations.push(
          'Ensure tenants have encryption keys generated on first use'
        );
      }
    } catch (error) {
      response.checks.keys.status = 'error';
      response.checks.keys.issues.push(
        `Failed to fetch key statistics: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      response.status = 'degraded';
    ...
    try {
      ...
      response.checks.rotation.tenantsNeedingRotation =
        rotationHealth.stats.keysNearExpiration;
      ...
      response.checks.rotation.issues.push(
        `Failed to check rotation health: ${
          error instanceof Error ? error.message : String(error)
        }`
      );
      response.status = 'degraded';
