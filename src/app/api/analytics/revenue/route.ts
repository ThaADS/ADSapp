...
export async function GET(request: NextRequest) {
  try {
    ...
    const growthRate = previousMRR > 0
      ? ((monthlyRecurringRevenue - previousMRR) / previousMRR) * 100
      : 0;

    // Calculate ARPU (Average Revenue Per User)
    const totalCustomers = (contacts || []).length;
    const averageRevenuePerUser = totalCustomers > 0
      ? monthlyRecurringRevenue / totalCustomers
      : 0;
    ...
    const churnRate = totalActiveSubscriptions > 0
      ? (cancelledSubscriptions / totalActiveSubscriptions) * 100
      : 0;
    ...
    const paybackPeriod = averageRevenuePerUser > 0
      ? Math.round(customerAcquisitionCost / averageRevenuePerUser)
      : 0;
...
function generateMonthlyRevenue(subscriptions: any[], range: string): Array<{
  month: string;
  revenue: number;
  subscriptions: number;
  customers: number;
  ...
  const monthlyData: Record<string, { revenue: number; subscriptions: Set<string>; customers: Set<string> }> = {};
