-- ============================================================================
-- TAGS MANAGEMENT SYSTEM MIGRATION
-- ============================================================================
...
-- Add category reference to tags table
...
CREATE INDEX idx_tags_name ON tags(organization_id, name);
CREATE INDEX idx_tags_active ON tags(organization_id, is_active) WHERE is_active = true;
CREATE INDEX idx_tags_sort_order ON tags(organization_id, sort_order);
...
CREATE INDEX idx_contact_tags_assigned_at ON contact_tags(assigned_at DESC);

CREATE INDEX idx_tag_categories_organization_id ON tag_categories(organization_id);
...
GRANT SELECT, INSERT, UPDATE, DELETE ON tag_categories TO authenticated;

-- ============================================================================
-- 9. MIGRATE EXISTING DATA
-- ============================================================================

-- Migrate existing tags from contacts.tags[] to new structure
DO $$
DECLARE
  org_record RECORD;
  ...
  default_colors TEXT[] := ARRAY[
    ...
    '#F59E0B:bg-amber-100 text-amber-800',  -- amber
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      -- For each tag in the contact's tags array
      ...
      LOOP
        ...
        IF tag_uuid IS NULL THEN
          ...
          color_pair := string_to_array(default_colors[(color_index % 8) + 1], ':');
        ...
        VALUES (contact_record.id, tag_uuid)
        ON CONFLICT (contact_id, tag_id) DO NOTHING;
      END LOOP;
    END LOOP;
  END LOOP;
...
-- ============================================================================
-- 10. INSERT DEFAULT TAGS FOR EACH ORGANIZATION
-- ============================================================================
...
-- ============================================================================
-- 11. CREATE DEFAULT TAG CATEGORIES
-- ============================================================================
...
-- ============================================================================
-- 12. OPTIONAL: VIEW FOR EASY TAG QUERIES
-- ============================================================================

-- Create a view for easy contact tag queries
...
GRANT SELECT ON contact_tags_view TO authenticated;
...
BEGIN
  ...
  RAISE NOTICE 'Migrated existing tags from contacts.tags[] to new structure';
