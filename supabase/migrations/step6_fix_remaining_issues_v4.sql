-- STEP 6 v4: Fix remaining Security Advisor issues
-- Drop ALL functions first to avoid conflicts

-- ============================================
-- PART 0: Drop ALL functions that need changes
-- ============================================
...
DROP FUNCTION IF EXISTS public.check_duplicate_pending_invitation();
DROP FUNCTION IF EXISTS public.update_team_member_count();
DROP FUNCTION IF EXISTS public.track_message_delivery();
DROP FUNCTION IF EXISTS public.update_drip_campaign_statistics();
DROP FUNCTION IF EXISTS public.refresh_ai_usage_analytics();
...
-- ============================================
-- PART 2: Recreate ALL Functions with search_path
-- ============================================
...
CREATE FUNCTION public.check_sla_breach()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
CREATE FUNCTION public.check_duplicate_pending_invitation()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
...
CREATE FUNCTION public.update_team_member_count()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
...
CREATE FUNCTION public.track_message_delivery()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'drip_campaigns') THEN
        EXECUTE $ex$
            CREATE FUNCTION public.update_drip_campaign_statistics()
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_matviews WHERE schemaname = 'public' AND matviewname = 'ai_usage_analytics') THEN
        EXECUTE $ex$
            CREATE FUNCTION public.refresh_ai_usage_analytics()
...
SELECT 'Step 6 v4 complete: All Security Advisor issues fixed!' AS status;
