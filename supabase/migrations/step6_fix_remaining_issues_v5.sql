-- STEP 6 v5: Fix remaining Security Advisor issues
-- Don't drop functions that have trigger dependencies - use CREATE OR REPLACE instead

-- ============================================
-- PART 0: Drop only functions WITHOUT trigger dependencies
-- ============================================
...
SELECT 'index' as type, sum(idx_blks_hit)::bigint as hits, sum(idx_blks_read)::bigint as reads,
    CASE WHEN sum(idx_blks_hit + idx_blks_read) > 0 THEN round(100.0 * sum(idx_blks_hit) / sum(idx_blks_hit + idx_blks_read), 2) ELSE 0 END as hit_rate
FROM pg_statio_user_indexes
UNION ALL
SELECT 'table' as type, sum(heap_blks_hit)::bigint as hits, sum(heap_blks_read)::bigint as reads,
    CASE WHEN sum(heap_blks_hit + heap_blks_read) > 0 THEN round(100.0 * sum(heap_blks_hit) / sum(heap_blks_hit + heap_blks_read), 2) ELSE 0 END as hit_rate
...
BEGIN
    ...
    ELSE
        EXECUTE $view$
            CREATE VIEW public.performance_slow_queries AS
            SELECT ''::text as query, 0::bigint as calls, 0::double precision as total_exec_time,
                   0::double precision as mean_exec_time, 0::bigint as rows WHERE false
        $view$;
    END IF;
END $$;
...
-- ============================================
-- PART 2A: CREATE OR REPLACE for trigger functions (have dependencies)
-- ============================================
...
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_sla_config RECORD; v_response_time INTERVAL;
BEGIN
    IF NEW.status = 'resolved' AND OLD.status != 'resolved' THEN
        SELECT * INTO v_sla_config FROM sla_configurations WHERE organization_id = NEW.organization_id AND is_active = true LIMIT 1;
        IF FOUND THEN
            v_response_time := NEW.resolved_at - NEW.created_at;
            IF v_response_time > v_sla_config.max_response_time THEN NEW.sla_breached = true; END IF;
        END IF;
    END IF;
    RETURN NEW;
...
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
    IF EXISTS (SELECT 1 FROM team_invitations WHERE organization_id = NEW.organization_id AND email = NEW.email AND status = 'pending' AND id != COALESCE(NEW.id, '00000000-0000-0000-0000-000000000000'::UUID)) THEN
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        UPDATE organizations SET team_member_count = (SELECT COUNT(*) FROM profiles WHERE organization_id = NEW.organization_id) WHERE id = NEW.organization_id;
    ELSIF TG_OP = 'DELETE' THEN
        UPDATE organizations SET team_member_count = (SELECT COUNT(*) FROM profiles WHERE organization_id = OLD.organization_id) WHERE id = OLD.organization_id;
    END IF;
...
BEGIN
    IF NEW.status IS DISTINCT FROM OLD.status THEN
        INSERT INTO message_delivery_logs (message_id, old_status, new_status, changed_at) VALUES (NEW.id, OLD.status, NEW.status, NOW());
    END IF;
    RETURN NEW;
EXCEPTION WHEN others THEN RETURN NEW;
END;
...
BEGIN
    IF TG_TABLE_NAME = 'drip_campaign_enrollments' THEN
        UPDATE drip_campaigns SET total_enrolled = (SELECT COUNT(*) FROM drip_campaign_enrollments WHERE campaign_id = NEW.campaign_id), updated_at = NOW() WHERE id = NEW.campaign_id;
    END IF;
    RETURN NEW;
EXCEPTION WHEN others THEN RETURN NEW;
END;
...
-- ============================================
-- PART 2B: CREATE for non-trigger functions (dropped above)
-- ============================================

CREATE FUNCTION public.log_invitation_event(
    p_invitation_id UUID, p_event_type VARCHAR, p_old_status VARCHAR DEFAULT NULL,
    p_new_status VARCHAR DEFAULT NULL, p_performed_by UUID DEFAULT NULL, p_metadata JSONB DEFAULT '{}'
...
CREATE FUNCTION public.log_api_key_event(
    p_api_key_id UUID, p_event_type VARCHAR, p_metadata JSONB DEFAULT '{}'
) RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
...
CREATE FUNCTION public.create_workflow_version(p_workflow_id UUID, p_changes JSONB DEFAULT '{}')
RETURNS UUID LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_version_id UUID; v_version_number INTEGER;
BEGIN
    SELECT COALESCE(MAX(version_number), 0) + 1 INTO v_version_number FROM workflow_versions WHERE workflow_id = p_workflow_id;
    INSERT INTO workflow_versions (workflow_id, version_number, changes) VALUES (p_workflow_id, v_version_number, p_changes) RETURNING id INTO v_version_id;
    RETURN v_version_id;
...
DECLARE v_score INTEGER := 0; v_contact RECORD; v_message_count INTEGER;
BEGIN
...
CREATE FUNCTION public.expire_old_invitations()
RETURNS INTEGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
...
DECLARE v_max_users INTEGER; v_current_users INTEGER;
BEGIN
    SELECT max_users INTO v_max_users FROM subscriptions WHERE organization_id = p_organization_id AND status = 'active' LIMIT 1;
...
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE v_enrollment RECORD; v_next_step RECORD;
BEGIN
    ...
    SELECT * INTO v_next_step FROM drip_campaign_steps WHERE campaign_id = v_enrollment.campaign_id AND step_order > v_enrollment.current_step ORDER BY step_order LIMIT 1;
    IF FOUND THEN
        UPDATE drip_campaign_enrollments SET current_step = v_next_step.step_order, next_message_at = NOW() + (v_next_step.delay_hours || ' hours')::INTERVAL WHERE id = p_enrollment_id;
    ELSE
        UPDATE drip_campaign_enrollments SET status = 'completed', completed_at = NOW() WHERE id = p_enrollment_id;
    END IF;
EXCEPTION WHEN others THEN NULL;
END;
$$;
...
BEGIN
    ...
    SELECT * INTO v_invitation FROM team_invitations WHERE token = p_token AND status = 'pending' AND expires_at > NOW();
    IF NOT FOUND THEN RETURN jsonb_build_object('success', false, 'error', 'Invalid or expired invitation'); END IF;
    UPDATE profiles SET organization_id = v_invitation.organization_id, role = v_invitation.role WHERE id = v_user_id;
...
BEGIN
    SELECT * INTO v_connection FROM crm_connections WHERE organization_id = p_organization_id AND status = 'active' LIMIT 1;
    IF NOT FOUND THEN RETURN jsonb_build_object('connected', false); END IF;
    RETURN jsonb_build_object('connected', true, 'provider', v_connection.provider, 'last_sync_at', v_connection.last_sync_at);
...
BEGIN
    RETURN QUERY SELECT cm.contact_id, cm.field_name, cm.local_value, cm.remote_value
    FROM crm_sync_conflicts cm JOIN contacts c ON cm.contact_id = c.id
...
BEGIN
    SELECT COUNT(*) INTO v_count FROM messages WHERE conversation_id = p_conversation_id AND is_read = false AND direction = 'inbound';
...
-- ============================================
-- PART 3: Conditional functions (only if tables exist)
-- ============================================
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_matviews WHERE schemaname = 'public' AND matviewname = 'ai_usage_analytics') THEN
        EXECUTE $ex$
            ...
            EXCEPTION WHEN others THEN REFRESH MATERIALIZED VIEW ai_usage_analytics;
            END;
            $func$
        $ex$;
    END IF;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'ai_settings') THEN
        EXECUTE $ex$
            ...
            DECLARE v_settings RECORD; v_current_spend NUMERIC; v_budget_percent NUMERIC;
            BEGIN
                SELECT * INTO v_settings FROM ai_settings WHERE organization_id = p_organization_id;
                IF NOT FOUND THEN RETURN jsonb_build_object('within_budget', true, 'spend', 0, 'budget', 0, 'percent', 0); END IF;
                SELECT COALESCE(SUM(cost_usd), 0) INTO v_current_spend FROM ai_responses WHERE organization_id = p_organization_id AND created_at >= date_trunc('month', NOW());
                v_budget_percent := CASE WHEN v_settings.monthly_budget_usd > 0 THEN (v_current_spend / v_settings.monthly_budget_usd) * 100 ELSE 0 END;
                RETURN jsonb_build_object('within_budget', v_current_spend < v_settings.monthly_budget_usd, 'spend', v_current_spend, 'budget', v_settings.monthly_budget_usd, 'percent', v_budget_percent, 'alert_threshold', v_settings.alert_threshold);
            EXCEPTION WHEN others THEN RETURN jsonb_build_object('within_budget', true, 'spend', 0, 'budget', 0, 'percent', 0);
            END;
            $func$
        $ex$;
    END IF;
...
SELECT 'Step 6 v5 complete: All Security Advisor issues fixed!' AS status;
