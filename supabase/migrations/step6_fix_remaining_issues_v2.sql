-- STEP 6 v2: Fix remaining Security Advisor issues
...
-- Recreate performance_index_usage with correct column names
...
-- Recreate performance_cache_hit_rate
...
-- Recreate performance_slow_queries (only if pg_stat_statements extension exists)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements') THEN
        ...
        $view$;
    ELSE
        -- Create empty placeholder view if extension not available
        EXECUTE $view$
            CREATE VIEW public.performance_slow_queries AS
            SELECT
                ''::text as query,
...
-- Recreate performance_table_sizes with correct syntax
...
-- ============================================
-- PART 3: Conditional functions (if tables exist)
-- ============================================
...
SELECT 'Step 6 v2 complete: Fixed Security Definer Views and Function Search Path warnings!' AS status;
