  ...
  scenarios: {
    ...
    moderate_user: {
      ...
      stages: options.stages.map(stage => ({
        ...
        target: Math.floor((stage.target * 0.35) / 0.4),
      })),
    ...
    light_user: {
      ...
      stages: options.stages.map(stage => ({
        duration: stage.duration,
        target: Math.floor((stage.target * 0.2) / 0.4),
      })),
    ...
    admin_user: {
      ...
      stages: options.stages.map(stage => ({
        duration: stage.duration,
        target: Math.floor((stage.target * 0.05) / 0.4),
      })),
...
function authenticate(email, password) {
  ...
  const response = http.post(`${BASE_URL}/api/auth/signin`, JSON.stringify({ email, password }), {
...
export function activeAgentScenario() {
  ...
  for (let i = 0; i < 30; i++) {
    group('Active Agent - Inbox Check', () => {
      ...
      if (i % 60 === 0) {
        const conversation = testConversations[Math.floor(Math.random() * testConversations.length)]
        makeAuthenticatedRequest('PUT', `conversations/${conversation.id}`, authData, {
      ...
      if (i % 120 === 0) {
        makeAuthenticatedRequest('GET', `contacts?search=${encodeURIComponent('test')}`, authData)
      }
    })
...
export function adminUserScenario() {
  ...
  for (let i = 0; i < 300; i++) {
    group('Admin - Activity', () => {
      ...
      if (i % 600 === 0) {
        makeAuthenticatedRequest('POST', 'analytics/export', authData, {
          format: 'csv',
